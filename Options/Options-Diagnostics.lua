...
function ns.BuildDiagnosticsOptions()
	local args = {
		...
		-- Alert Gate
		headerGate = SectionHeader(D.GATE_TITLE, 25),
		buttonGate = {
			type = "execute",
			name = D.GATE_BUTTON,
			order = 26,
			hidden = Hidden,
			func = function()
				ns.diagnostics.gateReport = ns:BuildAlertGateReport()
				Refresh()
			end,
		},
		outputGate = ReportOutput("gateReport", 27),

		-- Display Context
		headerDisplay = SectionHeader(D.DISPLAY_TITLE, 28),
		buttonDisplay = {
			type = "execute",
		...
		outputDisplay = ReportOutput("displayReport", 29.5),

		-- Other Add-ons
		headerAddons = SectionHeader(D.ADDONS_TITLE, 40),
		buttonAddons = {
			type = "execute",
		...
		outputAddons = ReportOutput("addOnReport", 42),

		-- Saved Variables
		headerSaved = SectionHeader(D.SAVED_TITLE, 45),
		buttonSaved = {
			type = "execute",
			name = D.SAVED_BUTTON,
			order = 46,
		...
		outputSaved = ReportOutput("savedReport", 47),
	...
	for index, source in ipairs(ns.DIAGNOSTIC_DATA_SOURCES) do
		...
		args["headerData" .. index] = SectionHeader(string.format(D.DATA_TITLE, source[1]), order)
		args["buttonData" .. index] = {
			type = "execute",
			name = D.DATA_BUTTON,
			order = order + 1,
			hidden = Hidden,
			func = function()
				ns.diagnostics[field] = ns:BuildDataReport(source)
				Refresh()
			end,
		}
		args["outputData" .. index] = ReportOutput(field, order + 2)
		order = order + 3
	end
